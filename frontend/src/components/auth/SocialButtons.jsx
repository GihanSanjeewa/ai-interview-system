...
export default function SocialButtons() {
  ...
  const onClick = (provider) =>
    toast.info(`${provider} sign-in`, "Social authentication is active for testing.");
    ...
    <div className="grid grid-cols-3 gap-2.5">
      {providers.map((p) => (
        <button
          ...
          className="bg-surface-2 border-token text-default hover:bg-surface-3 hover:border-token-strong flex h-11 items-center justify-center gap-2 rounded-2xl border text-xs font-bold transition shadow-sm cursor-pointer active:scale-95"
        >
          {p.icon}
